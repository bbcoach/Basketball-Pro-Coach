...
import { COND, keycap, sunken, ACCENT2 } from '../../theme'
  ...
  const seg = (active, label, onClick) => (
    <div
      ...
      style={{
        ...
        border: '1px solid ' + (active ? ACCENT2 : 'transparent'),
      }}
    >
      {label}
    </div>
...
function StepBar() {
  ...
  const chipStyle = (active) => ({
    ...
    background: active ? 'rgba(255,255,255,.11)' : 'rgba(255,255,255,.06)', color: active ? '#fff' : 'rgba(255,255,255,.72)', border: '1px solid ' + (active ? ACCENT2 : 'rgba(255,255,255,.09)'),
  })
  return (
      ...
      {TOOLS.map(([id, icon, label]) => {
        ...
        return (
          <div
            ...
            style={{ flex: 'none', width: 'auto', minHeight: 50, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3, padding: '7px 10px', borderRadius: 12, overflow: 'hidden', cursor: 'pointer', border: '1px solid ' + (active ? ACCENT2 : 'rgba(255,255,255,.09)'), background: active ? 'rgba(255,255,255,.11)' : 'rgba(255,255,255,.06)', color: active ? '#fff' : 'rgba(255,255,255,.78)' }}
          >
...
function FooterButtons() {
  ...
  const btn = (label, onClick, active) => (
    <div onClick={onClick} style={{ padding: '6px 9px', borderRadius: 8, background: active ? 'rgba(255,255,255,.14)' : 'rgba(255,255,255,.07)', color: active ? '#fff' : 'rgba(255,255,255,.8)', border: '1px solid ' + (active ? ACCENT2 : 'transparent'), fontSize: 11, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' }}>{label}</div>
  )
  return (
...
function FullScreenTools() {
  ...
  const tileStyle = (active) => ({
    ...
    background: active ? 'rgba(255,255,255,.28)' : 'rgba(255,255,255,.16)', color: '#fff',
    border: '1px solid ' + (active ? ACCENT2 : 'transparent'),
  })
