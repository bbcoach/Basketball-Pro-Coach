...
import { COND, keycap, field, centred, ACCENT2 } from '../../../theme'
    ...
    return (
      <div
        ...
        style={{
          ...
          background: active ? 'rgba(255,255,255,.11)' : 'rgba(255,255,255,.06)', color: active ? '#fff' : 'rgba(255,255,255,.55)',
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 80, background: 'rgba(9,9,11,.96)', display: 'flex', flexDirection: 'column', padding: '60px 20px 34px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, paddingBottom: 10 }}>
        ...
        <div onClick={closeSheet} style={{ padding: '7px 13px', borderRadius: 12, background: 'rgba(255,255,255,.09)', color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>Done</div>
      </div>
      <div style={{ display: 'flex', gap: 6, paddingBottom: 10 }}>
        <div onClick={openImport} style={{ flex: 1, textAlign: 'center', padding: '8px 10px', borderRadius: 12, background: 'rgba(255,255,255,.07)', color: 'rgba(255,255,255,.75)', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>Import</div>
      </div>
      <div style={{ display: 'flex', gap: 6, paddingBottom: 12 }}>
        ...
        {seg('play', 'Play')}
        {seg('drill', 'Drill')}
        <div onClick={savePlay} style={{ ...centred, padding: '9px 14px', borderRadius: 12, ...keycap(), color: '#101012', fontSize: 13, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' }}>Save</div>
      </div>
