...
export default function Court() {
  ...
  const { players, ball, t, step, playing, timeout, exportGhost, sel, view } = state
  ...
  const { tokens, routes, caps, badges } = useMemo(() => {
    ...
    all.forEach((ent) => {
      actsOf(ent).forEach((act) => {
        ...
        if (!exportGhost && act.step !== editStep) return
        const ty = act.type
  ...
  }, [players, ball, t, step, playing, timeout, exportGhost, sel, nSteps])
