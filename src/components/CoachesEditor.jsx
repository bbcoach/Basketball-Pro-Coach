  ...
  return (
    <div style={{ flex: 'none', display: 'flex', flexDirection: 'column', padding: '14px 18px 0' }}>
      ...
      <div style={{ display: 'flex', gap: 6, paddingBottom: 10 }}>
        ...
        />
        <div onClick={addCoach} style={{ padding: '10px 14px', borderRadius: 12, background: ACCENT, color: '#101012', fontSize: 13, fontWeight: 700, cursor: 'pointer', flex: 'none' }}>{coachEditId ? 'Save' : 'Add'}</div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 5, paddingBottom: 10 }}>
        {coaches.map((c) => (
          <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 10px', borderRadius: 12, background: coachEditId === c.id ? 'rgba(255,255,255,.11)' : 'rgba(255,255,255,.05)', border: '1px solid ' + (coachEditId === c.id ? ACCENT : 'rgba(255,255,255,.08)') }}>
